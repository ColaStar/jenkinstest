...
export function registerElement(element) {
	...
	DOM_MODULE.id = kebabCase(element.prototype.constructor.name) || element.prototype.elementName || element.elementName || element.prototype.is || element.is;
	element.prototype.is = element.is = DOM_MODULE.id;

	const hasBeforeRegister = element.prototype.hasOwnProperty("beforeRegister");

	if (!hasBeforeRegister) {

		// Build beforeRegister from the prototype getters.
		const properties = element.prototype.properties;
		const hostAttributes = element.prototype.hostAttributes;
		const observers = element.prototype.observers;
		const listeners = element.prototype.listeners;
