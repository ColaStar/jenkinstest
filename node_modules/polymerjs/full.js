...
Polymer.Base._addFeature({
    _prepTemplate: function () {
        if (!this.hasOwnProperty('_template')) {
            this._template = Polymer.DomModule.import(this.is, 'template') || this._template;
        }
        ...
        if (this._template) {
            this.__template = this._processTemplate(this._template.cloneNode(true));
        }
    },
    ...
    _stampTemplate: function () {
        ...
        if (template) {
            this.root = this.instanceTemplate(template);
        }
    },
...
Polymer.Annotations = {
    parseAnnotations: function (template) {
        ...
        if (!content._notes) {
            content._notes = [];
            this._parseNodeAnnotations(content, content._notes, template.hasAttribute('strip-whitespace'));
        }
...
Polymer.Base._addFeature({
    _prepAnnotations: function () {
        ...
        if (!template) {
            ...
            if (template._content && template._content._notes) {
                this._notes = ttemplate._content._notes;
            } else {
                this._notes = Polymer.Annotations.parseAnnotations(template);
...
Polymer.StyleTransformer = function () {
    ...
    var api = {
        ...
        elementStyles: function (element, callback) {
            ...
            var cssText = '';
            var ext = this.elementExtendsNative(element);
            for (var i = 0, l = styles.length, s; i < l && (s = styles[i]); i++) {
                var rules = styleUtil.rulesForStyle(s);
                cssText += nativeShadow ? styleUtil.toCssText(rules, callback) : this.css(rules, element.is, ext, callback, element._scopeCssViaAttr) + '\n\n';
            }
            return cssText.trim();
        },
        elementExtendsNative: function (element) {
            return element.__extendsNativeElement;
        },
...
Polymer.StyleProperties = function () {
    ...
    return {
        ...
        transformStyles: function (element, properties, scopeSelector) {
            var self = this;
            var ext = styleTransformer.elementExtendsNative(element);
            var hostSelector = styleTransformer._calcHostScope(element.is, ext);
            var rxHostSelector = ext ? '\\' + hostSelector.slice(0, -1) + '\\]' : hostSelector;
...
(function () {
    ...
    Polymer({
        ...
        _apply: function (deferProperties) {
            var e = this.__appliedElement || this;
            if (this.include) {
                e.textContent = styleUtil.cssFromModules(this.include, true, true) + e.textContent;
            }
            if (e.textContent) {
